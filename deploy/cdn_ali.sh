...
cdn_ali_deploy() {
  ...
  _certnamestr=$DEPLOY_CDN_Ali_Prefix'-'$(sha1sum "$_ccert" | cut -c1-20)
  ...
  _debug "Set Cert"
  _set_cert_query "$(_ali_urlencode "$DEPLOY_CDN_Ali_Prefix")" "$(_ali_urlencode "$_certnamestr")" "$_certstr" "$_keystr" && _ali_rest "Set Cert"
  return 0
}
